...
class image_converter:
    ...
    def __init__(self):
        ...
        self.image_sub = rospy.Subscriber('/tello/image_raw/compressed', Image, self.callback)
        self.takeoff_sub = rospy.Subscriber('/tello/takeoff', Empty, self.isTakeoff)
        self.land_sub = rospy.Subscriber('/tello/land', Empty, self.isLand)
    ...
    def line_detect(self, cv_image):
        ...
        mask = cv2.inRange(cv_image, (0, 0, 0), (255, 255, 255))
        ...
        if len(contours_blk) > 0 and cv2.contourArea(contours_blk[0]) > 5000:
            ...
            cv2.putText(cv_image, "Error: " + str(error), (10, 240), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2,
                        cv2.LINE_AA)
            cv2.line(cv_image, (int(x_min), 200), (int(x_min), 250), (0, 0, 0), 3)
    ...
    def callback(self, data):
        ...
        cv2.putText(cv_image, "battery: ", (570, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
